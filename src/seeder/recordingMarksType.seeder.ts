...
export class RecordingsMarkTypeSeeder implements Seeder {
  ...
  async drop(): Promise<any> {
    return await this.recordingsMarkTypeRepository.query(
      `DELETE FROM recordings_mark_type WHERE id <= 4`,
    );
  }
  ...
  async seed(): Promise<any> {
    const recordingMarkTypes: RecordingsMarkType[] = [
      ...
      { id: '2', type: 'FINISH_VIDEO_CALL', typeTitle: 'call finished' },
      {
        ...
        typeTitle: 'start recording video call',
      },
      {
        id: '4',
        type: 'STOP_RECORDING',
        typeTitle: 'stop recording video call',
      },
    ];

    // Insert into the database.
    const marksTypes =
      this.recordingsMarkTypeRepository.create(recordingMarkTypes);

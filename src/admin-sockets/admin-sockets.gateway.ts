...
@WebSocketGateway()
export class AdminSocketsGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  registerToRoomsUpdates(@ConnectedSocket() client: Socket) {
    this.roomsSubscriptions[client.id] =
      this.agentsConnectionsService.rooms$.subscribe((rooms) => {
  ...
  registerToPriorityLineUpdates(@ConnectedSocket() client: Socket) {
    this.priorityLineSubscriptions[client.id] = combineLatest([
      ...
      ...this.guestConnectionsService.priorityLine.map((pl) =>
        pl.priorityLine.asObservable(),
      ),
    ])
      .pipe(
        map(([mainPriorityLine, ..._]) =>
          mainPriorityLine.map((pl) => ({
            gender: pl.gender,

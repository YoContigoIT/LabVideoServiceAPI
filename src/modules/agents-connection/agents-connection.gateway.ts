...
export class AgentsConnectionGateway
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  constructor(
    ...
    @Inject(forwardRef(() => GuestsConnectionGateway))
    private guestsConnectionGateway: GuestsConnectionGateway,
  ...
  async handleDisconnect(
    ...
    if (room.sessionId) {
      console.log('----- room.sessionId - OPENVIDU -----');
      console.log(room.sessionId);
      console.log('-------------------------------------');

      try {
        const OVSession = await this.videoServiceService.getSessionById(
          room.sessionId,
        );
        ...
        room?.users.forEach(async (user) => {
          this.guestsConnectionService.updateGuestConnection(
            user.guestConnectionId,
            {
              endTimeConnection: new Date(),
            },
          );
          const callRecordId =
            await this.callRecordService.findCallRecordByGuestConnectionId(
              room.users[0].guestConnectionId,
            );
          if (callRecordId) this.callRecordService.update(callRecordId.id);

          this.guestsConnectionGateway.server
            .to(user.socketId)
            .emit('agent-disconnected', {});
        });

        await OVSession.fetch();
        if (OVSession?.connections?.length)
          await OVSession.close().catch((e) => {
    ...
    await this.agentsConnectionService.saveAgentDisconnection(
      room.host.agentConnectionId,
    );
  }

  @SubscribeMessage('connect-agent')
  async connectAgent(
    @MessageBody() createAgentsConnectionDto: CreateAgentsConnectionDto,
    @ConnectedSocket() client: Socket,
  ) {
    createAgentsConnectionDto.ip =
      (client.handshake.headers['x-forwarded-for'] as string) ||
      client.handshake.address;

    const agent = await this.agentsService.findOne(
      createAgentsConnectionDto.agent as any,
    );
    ...
    const room = this.agentsConnectionService.getRoomByAgentUUID(
      createAgentsConnectionDto.agent.uuid,
    );
    ...
    if (room) {
      ...
      throw new WsException({
        message: 'The Agent is already connect',
        error: 'ALREADY_CONNECTED',
      });
    }

    const agentConnection = await this.agentsConnectionService.agentConnection(
      createAgentsConnectionDto,
    );
    ...
    console.log('--------- rooms --------');
    console.log(this.agentsConnectionService.rooms);
    console.log('------------------------');
  ...
  async connectCall(
    @MessageBody() createVideoServiceDto: CreateVideoServiceDto,
    @ConnectedSocket() client: Socket,
  ) {
    ...
    if (room?.users?.length) {
      await this.guestsConnectionService.updateGuestConnection(
        room.users[0].guestConnectionId,
        {
          answer: new Date(),
        },
    ...
    const recordingInfo = await this.recordingService.create({
    ...
    this.agentsConnectionService.updateSessionIdOnRoom(
      session.sessionId,
      room.name,
    );
  ...
  queueGuestReconnect(
    @MessageBody() refuseCallDto: RefuseCallDto,
    @ConnectedSocket() socket: Socket,
  ) {
    const room = this.agentsConnectionService.getRoomByHostSocket(socket.id);
    if (!room) return;
    ...
    if (refuseCallDto.requeue) {
      const priorityLine =
        this.guestsConnectionService.findProperPriorityList(guest);

      this.guestsConnectionService.pushToAssertivePriorityLine(
        guest,
      ...
      this.guestsConnectionGateway.server
        .to(guest.socketId)
        .emit('disconnect-guest', { reason: 'CALL_REFUSED' });
    }
  ...
  async toggleAudioGuest(
    @MessageBody() toggleAudioGuestData: any,
    @ConnectedSocket() socket: Socket,
  ) {
    ...
    room.users.forEach((user) => {
      this.guestsConnectionGateway.server
        .in(room.name)
        .to(user.socketId)
        .emit('mute-audio', { toggleAudio: toggleAudioGuestData });
    });
  ...
  async reloadAgentVideoCall(@ConnectedSocket() socket: Socket) {
    try {
      ...
      const sessionId = await this.videoServiceService.getSessionById(
        room.sessionId,
      );
      if (!sessionId) return;
  ...
  async closeVideoCall(@ConnectedSocket() client: Socket) {
    ...
    room.users?.forEach((user) => {
      this.guestsConnectionGateway.server
        .in(room.name)
        .to(user.socketId)
        .emit('disconnect-guest', 'disconnect from server');
    });
